...
def image_to_ascii(image_path, asci_list=LIGHT_REVERSE, new_width=width, retainAspect=True, color=True, true_color=True):
...
width_ter, height_ter = shutil.get_terminal_size()
def video_to_ascii(video_path, ascii_list=LIGHT_REVERSE, new_width=width_ter, fps=60, retain_aspect=False, color=True, true_color=True):
    previous_frame = [0, 0]
...
if __name__ == "__main__":
    try:
        width_ter, height_ter = shutil.get_terminal_size()
        video_path = 'media/hearts.mp4'
        video_to_ascii(video_path, LIGHT_REVERSE, width_ter, 60, True, True, True)
